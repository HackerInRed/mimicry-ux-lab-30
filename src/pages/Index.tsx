...
const ThinkingDots = () => <div className="flex space-x-2 items-center p-4 bg-secondary rounded-2xl message-animation">
...
const Index = () => {
  ...
  return <div className="min-h-screen flex flex-col items-center justify-center bg-background p-4 md:p-6">
      ...
      <div className="w-full max-w-4xl glass-morphism rounded-2xl overflow-hidden">
        <div className="h-[500px] overflow-y-auto p-6 space-y-4 scrollbar-hide">
          {messages.map((message, index) => <div key={index} className={`flex ${message.isBot ? "justify-start" : "justify-end"}`}>
              <div className={`flex items-start space-x-2 max-w-[80%] message-animation ${message.isBot ? "bg-secondary" : "bg-white/10"} rounded-2xl p-4`}>
                {message.isBot && <MessageSquare className="w-5 h-5 mt-1 shrink-0" />}
        ...
        <form onSubmit={handleSubmit} className="border-t border-white/10 p-4 input-animation">
          <div className="flex items-center space-x-4">
            
            <div className="flex-1 bg-secondary rounded-xl px-4 py-3 flex items-center">
              <input type="text" value={input} onChange={e => setInput(e.target.value)} placeholder="Type your question here..." disabled={isLoading} className="flex-1 bg-transparent text-sm md:text-base focus:outline-none disabled:cursor-not-allowed" />
              <button type="submit" disabled={isLoading} className={`ml-2 text-white/70 hover:text-white transition-colors ${isLoading ? "opacity-50 cursor-not-allowed" : ""}`}>
